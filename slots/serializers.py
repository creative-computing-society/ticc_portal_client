...
#     class Meta:
#         model = Leave
#         fields = ['id', 'counsellor', 'date', 'description']
...
#         if Leave.objects.filter(counsellor=counsellor, date=date).exists():
#             raise serializers.ValidationError('Leave already exists for this counsellor on this date.')
#         #check if start date is in the future or today
#         if date < timezone.now().date():
#             raise serializers.ValidationError('Start date must be in the future or today')

#         if Holiday.objects.filter(date=date).exists():
#             raise serializers.ValidationError('Leave cannot be created on a holiday')
...
#                 # if slot.slots_booked == 4:
#                 #     pass
#                 # elif slot.slots_booked == 3:
#                 #     pass
#                 slot.capacity == 2
